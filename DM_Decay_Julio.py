    ...
    #DM a q q_bar y gluón
        ...
        for j in range(0,3):
            if m > np.max([fmass[j], mv[0]]):
                gamma += 3.*GF*suma*mv[0]**2*m**3/(2.*np.sqrt(2)*(4.*np.pi)**3)
        ...
        for j in range(0,9):
            if m > np.max([fmass[j], mv[1]]):
                if j== 3 or j==5 or j==7:
                    ...
                    suma2 = np.sum(gau**2)+np.sum(gvu**2)
                gamma += 3.*GF*suma2*mv[1]**2*m**3/(8.*np.cos(thetaW)**2*np.sqrt(2)*(4.*np.pi)**3)
    
    #DM a 2 higgs y 2W            
    elif i == 8 and m > 2*mh+2*mv[0]:
        gamma = mv[0]**4*m**3/(15.*(8*np.pi)**5*v**4)

    #DM a 2 higgs y 2Z    
    elif i == 9 and m > 2*mh+2*mv[1]:
        gamma = mv[1]**4*m**3/(30.*(8*np.pi)**5*v**4)
...
plt.legend()
plt.grid()
plt.savefig('DM_Decay_1.1.1.png')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig('DM_Decay_1.2.1.png')
plt.show()
