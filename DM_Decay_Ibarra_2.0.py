...
for i in range(0,len(DMMass)):
    ...
    for j in range(0,12,2):
            if m > np.max([fmass[j], mv[0]]):
                Gamma[5,i] = 3.*GF*suma*m**5/(2.*np.sqrt(2)*(4.*np.pi)**3)
...
for i in range(0,len(DMMass)):
    ...
    for j in range(0,12):
            if m > np.max([fmass[j], mv[1]]):
                Gamma[6,i] = 3.*GF*(ga[j]**2+gv[j]**2)*m**5/(2.*np.sqrt(2)*(4.*np.pi)**3)
...
for i in range(0,len(DMMass)):
    m = DMMass[i]
    if m > 2*mv[0]+2*mh:
        Gamma[7,i] = m**7/(15.*(8*np.pi)**5*v**4)
...
for i in range(0,len(DMMass)):
    m = DMMass[i]
    if m > 2*mv[1]+2*mh:
        Gamma[8,i] = m**7/(30.*(8*np.pi)**5*v**4)
...
plt.legend()
plt.grid()
plt.savefig('DM_Decay_2.1.2.png')
plt.show()
...
plt.legend()
plt.grid()
plt.savefig('DM_Decay_2.2.2.png')
plt.show()
